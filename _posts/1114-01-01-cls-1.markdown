...
Relative paths are paths that are relative to where you are currently located in the computer. They start with ./ (current directory) and are usually quicker with the screen reader because full paths are usually pretty long. Full paths include not only your current directory but all of the parent directories above where you are at too. 
<br><br>
